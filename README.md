...
##作成先リポジトリは HandoffBox 固定##
- `gh issue create` の `-R` は **必ず** `comodoidea-lab/HandoffBox` にする
- 作業中のプロジェクトリポジトリ（例: tech-notes, markup-studio）には **作成しない**
- `-R` を省略しない。省略すると cwd のリポジトリに作成され事故る
- ユーザー指示・本文の Context に別リポジトリ名があっても、Issue の作成先は HandoffBox
...
- 作成先は HandoffBox のみ（`-R comodoidea-lab/HandoffBox` を厳守。他リポジトリに書き換えない）
...
```
HandoffBox（ https://github.com/comodoidea-lab/HandoffBox ）の README を読んで、Handoff Issue を1つ作成して。##GitHub公式CLIを使用してログイン## ##作成先リポジトリは HandoffBox 固定##
```

### gh CLI example / 作成例

**`-R comodoidea-lab/HandoffBox` 固定。** 作業リポジトリ名に置き換えない。
